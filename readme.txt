=== Plugin Name ===
Contributors: skapator, alexitsios
Donate link: http://wpkamikaze.com/
Tags: bootstrap, widget
Requires at least: 3.6
Tested up to: 3.9.1
Stable tag: 1.0.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Adds a menu on the bottom of wp widgets where you can set bootstrap columns or rows

== Description ==

Adds a menu on the bottom of wp widgets in the admin widgets area where you can set bootstrap columns or rows.

You have to use Bootstrap framework as this plugin does not provide the bootstrap files.

Classes are added to the 'before_widget' arg on register_sidebars.

### Features

* Select to open a div.row
* Select to close a div.row
* Select div.col-lg, div.col-md, div.col-sm, div.col-xs, columns.
* Select hidden-lg, hidden-md, hidden-sm, hidden-xs.

Here's a link to our website [www.wpkamikaze.com](http://wpkamikaze.com/ "WordPress Themes and plugins").

== Installation ==

1. Upload the files to the /wp-content/plugins/wpkmkz-bootstrap-grid-widgets/ directory.
2. Activate the plugin through the 'Plugins' menu in WordPress

== Frequently Asked Questions ==
-

== Screenshots ==
1. You will see a button in the bootom of your widegts where you can set cols and rows and hidden classes.

== Changelog ==

= 1.0 =

== Upgrade Notice ==

= 1.0 =
Release
